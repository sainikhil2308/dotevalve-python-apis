...
def process_dir(root_dir,curr_dir,args,template=None,tuning_config=CONFIG_DEFAULTS,evaluation_config=None,):
...
def show_template_layouts(omr_files, template, tuning_config):
    for file_path in omr_files:
        ...
        directory = os.path.dirname("layouts/" + file_path)
        ...
        output_file_path = os.path.join(directory, f"{file_name}_template_layout.jpg")
        cv2.imwrite(output_file_path, template_layout)

        print(f"Template layout saved to: {output_file_path}")
...
def process_files(
    ...
    for file_path in omr_files:
        ...
        if multi_marked == 0 or not tuning_config.outputs.filter_out_multimarked_files:
            ...
            #     pass
    print("omr_response",omr_response)
